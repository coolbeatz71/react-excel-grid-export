...
var excelSheetFromDataSet = function excelSheetFromDataSet(dataSet) {
  ...
  */
  ...
  dataSet.forEach(function (dataSetItem) {
    ...
    if (columnsWidth.length > 0) ws["!cols"] = columnsWidth;
  ...
  if (range.s.c < 10000000) ws["!ref"] = _tempaXlsx2.default.utils.encode_range(range);
...
function fixRange(range, R, C, rowCount, xSteps, ySteps) {
  if (range.s.r > R + rowCount) range.s.r = R + rowCount;
  if (range.s.c > C + xSteps) range.s.c = C + xSteps;
  if (range.e.r < R + rowCount) range.e.r = R + rowCount;
  if (range.e.c < C + xSteps) range.e.c = C + xSteps;
}
